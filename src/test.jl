...
#                 sphericalWaveExcitationNormalDeriv,
#                 wavenumber)
# exportSourcesGmsh(mesh_filename, "sourcesIE_cube_real", real.(sourcesIE))
# exportSourcesGmsh(mesh_filename, "sourcesIE_cube_mag", abs.(sourcesIE))
# exportSourcesGmsh(mesh_filename, "sourcesIEnd_cube_real", real.(sourcesIEnd))
# exportSourcesGmsh(mesh_filename, "sourcesIEnd_cube_mag", abs.(sourcesIEnd))
# exportSourcesGmsh(mesh_filename, "scale_factor_cube_real", real.(sourcesIEnd ./ sourcesIE))
# exportSourcesGmsh(mesh_filename, "scale_factor_cube_mag", abs.(sourcesIEnd ./ sourcesIE))
# exportSourcesGmsh(mesh_filename, "rel_error_cube_real", real.((sourcesIE - sourcesIEnd) ./ sourcesIE))
# exportSourcesGmsh(mesh_filename, "rel_error_cube_mag", abs.((sourcesIE - sourcesIEnd) ./ sourcesIE))
# l2norm_error = norm(sourcesIE .- sourcesIEnd) / norm(sourcesIE)
...
l, m = 0, 0
lambda=5.0
src_quadrature_rule = gauss79rule
test_quadrature_rule = gauss1rule
wavenumber = 2*pi/lambda + 0*im
sphericalWaveExcitation(x_test, y_test, z_test) = sphericalWave(excitation_amplitude, wavenumber, [x_test,y_test,z_test], l, m)
sphericalWaveExcitationNormalDeriv(x_test, y_test, z_test, normal) = sphericalWaveNormalDerivative(excitation_amplitude, wavenumber, [x_test,y_test,z_test], l, m, normal)
...
#### POST CHANGING SCALAR GREENS NORMAL DERIV BUG ####

# wavenumber = 2*pi/lambda + 0*im
# wavevector = [wavenumber, 0.0, wavenumber] ./ sqrt(2)
#
# println("\nQ2.1: How does the soft IE compare to the soft IE normal deriv when the mesh is a sphere r=1 centered at origin w/ spherical wave lambda=3?")
# mesh_filename = "examples/simple/sphere/sphere_1m_1266.msh"
# l, m = 0, 0
# excitation_amplitude = 1.0
# lambda=3.0
# src_quadrature_rule = gauss7rule
...
#                 sphericalWaveExcitationNormalDeriv,
#                 wavenumber)
# exportSourcesGmsh(mesh_filename, "sourcesIE_sphere_1m_real", real.(sourcesIE))
# exportSourcesGmsh(mesh_filename, "sourcesIE_sphere_1m_imag", imag.(sourcesIE))
# exportSourcesGmsh(mesh_filename, "sourcesIE_sphere_1m_mag", abs.(sourcesIE))
# exportSourcesGmsh(mesh_filename, "sourcesIEnd_sphere_1m_real", real.(sourcesIEnd))
# exportSourcesGmsh(mesh_filename, "sourcesIEnd_sphere_1m_imag", imag.(sourcesIEnd))
# exportSourcesGmsh(mesh_filename, "sourcesIEnd_sphere_1m_mag", abs.(sourcesIEnd))
# exportSourcesGmsh(mesh_filename, "scale_factor_sphere_1m_real", real.(sourcesIEnd ./ sourcesIE))
# exportSourcesGmsh(mesh_filename, "scale_factor_sphere_1m_mag", abs.(sourcesIEnd ./ sourcesIE))
# exportSourcesGmsh(mesh_filename, "rel_error_sphere_1m_real", real.((sourcesIE - sourcesIEnd) ./ sourcesIE))
# exportSourcesGmsh(mesh_filename, "rel_error_sphere_1m_mag", abs.((sourcesIE - sourcesIEnd) ./ sourcesIE))
# l2norm_error = norm(sourcesIE .- sourcesIEnd) / norm(sourcesIE)
# avg_scale = sum(abs.(sourcesIEnd ./ sourcesIE))/pulse_meshIE.num_elements
# println("Average scale factor = ",avg_scale)
# println("Q2.1 Error = ", l2norm_error)
# #
# println("\nQ2.2: How does the soft IE compare to the soft IE normal deriv when the mesh is a sphere r=1 centered at origin w/ spherical wave lambda=6?")
# mesh_filename = "examples/simple/sphere/sphere_1m_1266.msh"
# l, m = 0, 0
# excitation_amplitude = 1.0
# lambda=6.0
...
# avg_scale = sum(abs.(sourcesIEnd ./ sourcesIE))/pulse_meshIE.num_elements
# println("Average scale factor = ",avg_scale)
# println("Q2.2 Error = ", l2norm_error)
#
# println("\nQ2.3: How does the soft IE compare to the soft IE normal deriv when the mesh is a sphere r=1 centered at origin w/ spherical wave lambda=9?")
# mesh_filename = "examples/simple/sphere/sphere_1m_1266.msh"
# l, m = 0, 0
# excitation_amplitude = 1.0
# lambda=9.0
...
# avg_scale = sum(abs.(sourcesIEnd ./ sourcesIE))/pulse_meshIE.num_elements
# println("Average scale factor = ",avg_scale)
# println("Q2.3 Error = ", l2norm_error)
#
# lambdas = [3, 6, 9, 12]
# scale_factors = [-0.450 - 0.957im, -2.533 - 2.662im, -5.971 - 4.175im, -10.782 - 5.650im]
# Plots.plot(lambdas, real.(scale_factors), label="Real Part", ylabel="analytical sources / soft IE normal deriv sources")
# Plots.plot!(lambdas, imag.(scale_factors), label="Imag Part", xlabel="Wavelength", legend = :right, size=(800,600))
# Plots.plot!(lambdas, abs.(scale_factors), label="Magnitude", title="Scale Factors of Sphere Problem")
# savefig("scale_factors_plot")
