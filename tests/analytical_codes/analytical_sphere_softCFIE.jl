...
for run_idx in 1:length(num_elements)
    ...
    mag_filename = string("sources_mag_softCFIE_sphere",num_elements[run_idx])
...
plot!(sqrt.(n), predicted_errors, title="1m Sphere vs Analytical Solution", label="Sound-Soft CFIE", xlabel="sqrt(N), N is number of unknowns", ylabel="l2-Error")
savefig("sphere_convergence_results_softCFIE")
println("Convergence rate = ", slope)

#Check if convergence rate is correct
convergence_rates = [-2.4131803618461456, -2.7143150389000463, 0] # using 2, 3, or 4 meshes, 7pnt src 1 pnt test
...
#output data
output_file = open("sphere_convergence_data_softCFIE.txt", "w")
