...
@testset "octree tests" begin
    ...
    @testset "fillOctreedZdkMatricesSoundSoft! tests" begin
        ...
        @test isapprox(octree.nodes[1].node2node_Z_matrices[1], dZdk_matrix, rtol = 1e-14)

        num_levels = 2
        octree = createOctree(num_levels, pulse_mesh)
        fillOctreedZdkMatricesSoundSoft!(pulse_mesh, octree, wavenumber, compression_distance, ACA_tol)
        @test isapprox(octree.nodes[2].node2node_Z_matrices[1], dZdk_matrix[[1,2],[1,2]], rtol = 1e-14)
        @test isapprox(octree.nodes[2].node2node_Z_matrices[2], dZdk_matrix[[1,2],[3,4]], rtol = 1e-14)
        @test isapprox(octree.nodes[2].node2node_Z_matrices[4], dZdk_matrix[[1,2],[7,8]], rtol = 1e-14)
        @test isapprox(octree.nodes[3].node2node_Z_matrices[1], dZdk_matrix[[3,4],[1,2]], rtol = 1e-14)
        @test isapprox(octree.nodes[4].node2node_Z_matrices[4], dZdk_matrix[[5,6],[7,8]], rtol = 1e-14)
        @test isapprox(octree.nodes[5].node2node_Z_matrices[3], dZdk_matrix[[7,8],[5,6]], rtol = 1e-14)

        num_levels = 3
        large_compression_dist = 1e10 # all interactions use direct Z calculation
        octree = createOctree(num_levels, pulse_mesh)
        fillOctreedZdkMatricesSoundSoft!(pulse_mesh, octree, wavenumber, large_compression_dist, ACA_tol)
        @test isapprox(octree.nodes[11].node2node_Z_matrices[5][1,1], dZdk_matrix[5,6], rtol = 1e-14)
        @test isapprox(octree.nodes[11].node2node_Z_matrices[8][1,1], dZdk_matrix[5,8], rtol = 1e-14)
        ...
        test_node = octree.nodes[11]
        src_node = octree.nodes[13]
        @test isapprox(test_node.node2node_Z_matrices[5][1,1], dZdk_matrix[5,6], rtol = 1e-14)
